...
namespace VotingSystem
{
    public partial class Voter : Form
    {
            ...
            else
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(sqlCon))
                {
                    try
                    {
                        ...
                        // Notify that the voter has been added
            ...
            if(dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if(confirmDel == DialogResult.Yes)
                {
                    using(MySqlConnection conn = new MySqlConnection(sqlCon))
                    {
                        ...
                        {
                            conn.Open();
                            string deleteQuery = "DELETE FROM voter WHERE username = @username";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                // selected user
                string origUsername = dataGridView1.SelectedRows[0].Cells["username"].Value.ToString();
                ...
                using(MySqlConnection conn = new MySqlConnection(sqlCon))
                {
                    try
                    {
                        ...
                        cmd.Parameters.AddWithValue("@origUsername", origUsername);
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            displayVoters(); // refreshes the data in the datagrid view
                        }
                        else
                        {
